...
class auto_control():
    def __init__(self):
        ...
        #前进速度PID控制
        self.forward_pid = PID(Kp=0.5, Ki=0.0004, Kd=0, setpoint=370.0, output_limits=(self.MIN_LIN_VEL_F, self.MAX_LIN_VEL_F))
        #左右飞行速度PID控制
    ...
    def track_call_back_match(self, center):
        if self.can_pass == 1:
            if center.is_person == 1 and center.conf > 0.5:
                self.loss_count = 0
                #yaw控制
